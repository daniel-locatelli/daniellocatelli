...
- **Multilingual content edits:** When editing a content file in one locale (e.g., `en/`), always check and update the corresponding files in the other locales (`pt/`, `de/`). Translatable fields (Country, Language, City names, etc.) must be localized. Structural fields (Link, DateStart, Place, Active, etc.) must be kept in sync. **Last updated:** every content page shows a "Last updated" date and emits it as `article:modified_time`/`dateModified`; by default it is the file's last git commit date. For a significant content revision, set `Updated: "YYYY-MM-DD"` in the frontmatter (it overrides the git date) and keep it identical across locales; do not bump it for typo fixes, reformatting, or asset changes.
