...
text: |
  ...
  ## The master thesis already lived this

  On-site gluing of timber panels: the structural checks took fabrication variables as inputs.
...
text: |
  ...
  July: a field guide to *compas* and *compas_timber*, the instrument this thesis runs on.

  **The codebase where the constraint modules will live is already mapped.**
notes: "One month in, concrete evidence of being inside the instrument rather than pointing at it. The field guide is the entry point for the constraint work that starts next."
---
...
{/*
  ...
  1 title, 4 flip, 5 chip precedent, 7 claim diagram, 12 question,
  16 timeline, 17 next steps.
*/}
...
<style>{`
  ...
  /* Body. The 50em measure exists so single-sentence lines fit on one
     ...
     sentence, not the CSS. (No text-wrap: balance here: it froze Chrome's
     renderer on this page; the base styles' text-wrap: pretty remains.) */
  #deck .slide-md p {
    font-family: "Poppins", sans-serif;
    font-weight: 300;
