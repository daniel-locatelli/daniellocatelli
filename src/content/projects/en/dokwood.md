...
![Three columns: AI clients such as chat interfaces, IDEs and agents on the left, the proposed DOKwood MCP server in the centre with its tools, resources, prompts and auth bridge, and the DOKwood platform on the right with its data (projects, buildups, layers, products), rules (requirements, calculations, certificates) and vocabulary (tenant dictionary, bSDD), connected by MCP and GraphQL arrows.](/assets/content/projects/dokwood/dokwood-mcp.svg)
